...
def LIF_network_fnc(model_path, scaling_factor, u, stims, downsample, use_initial_weights):
    ...
    W = torch.tensor(w / scaling_factor, dtype=torch.float32, device=device)
    ...
    # Input stimulus
    u = torch.tensor(u[:, ::downsample], dtype=torch.float32, device=device)
    w_in_tensor = torch.tensor(w_in, dtype=torch.float32, device=device)
    ...
    nt = int(T / dt)           # total number of points in a trial
    ...
    if len(taus.flatten()) > 1:
        td = (1.0 / (1 + np.exp(-taus_gaus)) * (taus[1] - taus[0]) + taus[0]) * 5 / 1000
        td0 = (1.0 / (1 + np.exp(-taus_gaus0)) * (taus[1] - taus[0]) + taus[0]) * 5 / 1000
        tr = 0.002
    else:
        td = taus * 5 / 1000
        td0 = td
    ...
    td = torch.tensor(td, dtype=torch.float32, device=device).flatten()
    tr = torch.tensor(tr, dtype=torch.float32, device=device)

    # Synaptic parameters
    IPSC = torch.zeros(N, device=device)      # post synaptic current storage variable
    h = torch.zeros(N, device=device)         # storage variable for filtered firing rates
    r = torch.zeros(N, device=device)         # second storage variable for filtered rates
    hr = torch.zeros(N, device=device)        # third variable for filtered rates
    JD = torch.zeros(N, device=device)        # storage variable required for each spike time
    ns = 0                 # number of spikes, counts during simulation

    v = vreset + torch.rand(N, device=device) * (30 - vreset)  # initialize voltage with random distributions
    ...
    REC = torch.zeros(nt, N, device=device)  # membrane voltage (in mV) values
    Is = torch.zeros(N, nt, device=device)  # input currents from the ext_stim
    IPSCs = torch.zeros(N, nt, device=device) # IPSC over time
    spk = torch.zeros(N, nt, device=device) # spikes
    rs = torch.zeros(N, nt, device=device)  # firing rates
    hs = torch.zeros(N, nt, device=device) # filtered firing rates

    # used to set the refractory times
    tlast = torch.zeros(N, device=device)
    ...
    for i in range(nt):
        ...
        stim_idx = min(int(i // 100), ext_stim.shape[1] - 1)
        I = I + ext_stim[:, stim_idx]
        Is[:, i] = ext_stim[:, stim_idx]

        # LIF voltage equation with refractory period
        dv = ((dt * i > tlast + tref).float() * (-v + I) / tm)  # linear LIF
        v = v + dt * dv + torch.randn(N, device=device) / 10
        ...
        tlast = tlast + (dt * i - tlast) * (v >= vpeak).float()
        ...
        if tr == 0:
            ...
            r = r * torch.exp(-dt / td) + (v >= vpeak).float() / td
            rs[:, i] = r
        else:
            ...
            hr = hr * torch.exp(-dt / tr) + (v >= vpeak).float() / (tr * td)
            rs[:, i] = r

        # record the spikes
        spk[:, i] = (v >= vpeak).float()
        
        v = v + (30 - v) * (v >= vpeak).float()

        # record the membrane voltage tracings from all the units
        REC[i, :] = v

        # reset with spike time interpolant implemented.
        v = v + (vreset - v) * (v >= vpeak).float()

    # Plot the population response
    w_out_tensor = torch.tensor(w_out / scaling_factor, dtype=torch.float32, device=device)
    out = torch.matmul(w_out_tensor, rs)

    # Compute average firing rate for each population (excitatory/inhibitory)
    inh_fr = torch.zeros(len(inh_ind), device=device)
    for i in range(len(inh_ind)):
        inh_fr[i] = torch.sum(spk[inh_ind[i], :] > 0).float() / T

    exc_fr = torch.zeros(len(exc_ind), device=device)
    for i in range(len(exc_ind)):
        exc_fr[i] = torch.sum(spk[exc_ind[i], :] > 0).float() / T

    all_fr = torch.zeros(N, device=device)
    for i in range(N):
        all_fr[i] = torch.sum(spk[i, 10:] > 0).float() / T
    ...
    return (W.cpu().numpy(), REC.cpu().numpy(), spk.cpu().numpy(), 
            rs.cpu().numpy(), all_fr.cpu().numpy(), out.cpu().numpy(), params) 